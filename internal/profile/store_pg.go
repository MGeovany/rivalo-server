...
const profileColumns = `id, display_name, preferred_position, height_cm, weight_kg, created_at, updated_at`
...
func (s *PostgresStore) Update(ctx context.Context, id string, u Update) (Profile, error) {
	...
	return scanProfile(s.pool.QueryRow(ctx, query, id, u.DisplayName, u.PreferredPosition, u.HeightCM, u.WeightKG))
}
